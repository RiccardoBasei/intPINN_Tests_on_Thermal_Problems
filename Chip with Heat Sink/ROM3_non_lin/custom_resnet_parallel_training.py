...
def parallel_train(model, model1, training_datasets, parameters, loss_fn, w, optimizer, dyn_sys_numpy, scheduler=None, shuffle_dataset=True, enable_parallel_training=False, update_epochs=50):
    ...
    for num_epochs, batch_size in zip(epochs_schedule, batching_schedule):
        ...
        for num_epoch in range(num_epochs):
            ...
            if (num_epoch == 500 or num_epoch == 1400) and scheduler == None:
                optimizer.param_groups[0]['lr'] *=0.5
            
            # check if the model has been updated correctely
            ans = compare_models(model, model1)
            if ans:
            ...
            for batch_data, batch_ss in zip(enumerate(train_dataloader), enumerate(ss_dataloader)):
                ...
                physics_res, minmax = f(model, model1, batch_data, batch_size, dynamical_system, enable_parallel_training)
...
def f(model, model1, inp, n_points, dynamical_system, enable_parallel_training):
    
    num_states = dynamical_system['A'].shape[0]
    
    # output of the net and its gradient
    if enable_parallel_training:
    ...
    T1 = torch.matmul(dynamical_system['C'], out1) + torch.transpose(torch.tile(dynamical_system['D'],(n_points,1)),0,1)
    T = torch.matmul(dynamical_system['C'], out) + torch.transpose(torch.tile(dynamical_system['D'],(n_points,1)),0,1)
    
    # temperature dependant characteristics evaluation
    try:
        f_T = np.tile(dynamical_system['f'](T1[0].detach().numpy()),(num_states,1))
    except RuntimeError:
        f_T = torch.tile(dynamical_system['f'](T1[0]),(num_states,1))
    coeff_T = dynamical_system['coeff'](T1[0])
    
    # variables taking into account non-linearity effects
    p_T = torch.multiply(inp[:,1], coeff_T)
    out_t_T = torch.multiply(out_t, torch.Tensor(f_T))
    
    # right-hand side
    Tamb_vec = 300/9*torch.ones(inp.shape[0])
    u = torch.vstack([Tamb_vec, p_T]) # note: for this problem, the second input (Tamb = 300/norm_u) is kept constant
    ...
    # left-hand side
    lhs = torch.matmul(dynamical_system['E'], out_t_T)
